...
class TLBEntry(val nSectors: Int, val superpage: Boolean, val superpageOnly: Boolean)(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  val vstage1 = Vec(nSectors, Bool())
  val vstage2 = Vec(nSectors, Bool())
...
class TLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  val superpage_hits = superpage_entries.map(_.hit(vpn, stage1_enbl & priv_v, stage2_enbl))
  val hitsVec = all_entries.map(vm_enabled && _.hit(vpn, stage1_enbl & priv_v, stage2_enbl))
  ...
  when (do_refill && !invalidate_refill) {
    ...
    newEntry.sw := pte.sw()
    newEntry.sx := pte.sx()
    newEntry.vr := Bool(usingHype) & pte_s2.ur()
    newEntry.vw := Bool(usingHype) & pte_s2.uw()
    newEntry.vx := Bool(usingHype) & pte_s2.ux()
    newEntry.pr := prot_r
    newEntry.pw := prot_w
    newEntry.px := prot_x
  ...
  io.ptw.req.bits.bits.do_stage1 := r_stage1_enbl
  io.ptw.req.bits.bits.do_stage2 := r_stage2_enbl
  io.ptw.req.bits.bits.priv_v := r_priv_v
